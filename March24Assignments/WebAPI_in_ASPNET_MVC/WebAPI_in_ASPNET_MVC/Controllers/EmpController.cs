...
namespace WebAPI_in_ASPNET_MVC.Controllers
{
    ...
    public class EmpController : ControllerBase
    {
        ...
        [HttpDelete]
        public async Task<ActionResult<List<Employee>>> DeleteeEmployee(int id)
        {
            var employee = await _context.employees.FindAsync(id);
            if (employee == null)
            {
        ...
        [HttpDelete("del2/{id}")]
        public async Task<ActionResult<Employee>> DeleteeEmployee2(int id)
        {
            var employee = await _context.employees.FindAsync(id);
            if (employee == null)
            {
